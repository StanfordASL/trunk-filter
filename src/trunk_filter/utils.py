...
# Transformations for inputs
def get_input_array_from_dataframe(df):
    links = sorted([int(re.findall(r'\d+', col)[0]) for col in df.columns if col.startswith('ux')])
    query = [f'{axis}{i}' for i in links for axis in ['ux', 'uy']]
    return df[query].to_numpy().reshape(-1, len(links), 2)

def get_dataframe_from_input_array(data_array, join_with=None):
    data = data_array.reshape(-1, 2 * data_array.shape[1])
    columns = [f'{axis}{i+1}' for i in range(data_array.shape[1]) for axis in ['ux', 'uy']]
